...
- 版本 1.1.1 增加字幕检测恢复：自动尝试开启 YouTube CC、字幕轨道失败时回退读取当前字幕 DOM，并提供重新扫描按钮。
- 版本 1.2.0 优化双字幕体验：固定两排显示、单行省略、译文同步出现、扩大缓存并提升预翻译速度。
- 版本 1.2.1 优化双字幕时间轴匹配：官方译文不再只按数组下标匹配，而是按字幕开始时间匹配，减少中文漏显。
